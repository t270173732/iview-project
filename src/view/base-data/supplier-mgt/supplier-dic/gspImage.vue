...
<script>
    ...
    export default {
        ...
        mounted() {
            if(this.$parent.supplierFormData.gspUrl) {
                let bdOrgPicVO = {};
                bdOrgPicVO.url = this.$config.img_base_url + this.$parent.supplierFormData.gspUrl;
            ...
            handleSuccess: function(res, file) {
                if(res.code == '200') {
                    ...
                    if(res.fileData.gspName){
                        this.$parent.supplierForm.updateData('gspName', res.fileData.gspName);
                    }
                    //地址
                    if(res.fileData.gspAddr){
                        this.$parent.supplierForm.updateData('gspAddr', res.fileData.gspAddr);
                    }
                    //有效期
                    if(res.fileData.gspValidTime){
                        this.$parent.supplierForm.updateData('gspValidTime', res.fileData.gspValidTime);
                    }
                    //认证范围
                    if(res.fileData.gspScope){
                        this.$parent.supplierForm.updateData('gspScope', res.fileData.gspScope);
                    }
                    //证书编号
                    if(res.fileData.gspNo){
                        this.$parent.supplierForm.updateData('gspNo', res.fileData.gspNo);
                    }
                    //发证机关
                    if(res.fileData.gspLicenseOrg){
                        this.$parent.supplierForm.updateData('gspLicenseOrg', res.fileData.gspLicenseOrg);
                    }
                    //发证日期
                    if(res.fileData.gspLicenseTime){
                        this.$parent.supplierForm.updateData('gspLicenseTime', res.fileData.gspLicenseTime);
                    }
                    ...
                    this.$parent.supplierFormData.gspImg = this.imageFile;
                } else {
                    this.warn(res.msg);
                }
                this.limitImage = false
