...
<script>
    ...
    export default {
        ...
        mounted() {
            if(this.$parent.storeInfoFormData.blUrl) {
                let bdOrgPicVO = {};
                bdOrgPicVO.url = this.$config.img_base_url + this.$parent.storeInfoFormData.blUrl;
            ...
            handleSuccess: function(res, file) {
                if(res.code == '200') {
                    ...
                    if(res.fileData.blName){
                        this.$parent.storeInfoForm.updateData('blName', res.fileData.blName);
                    }
                    //主体类型
                    if(res.fileData.blType){
                        this.$parent.storeInfoForm.updateData('blType', res.fileData.blType);
                    }
                    //地址
                    if(res.fileData.blAddr){
                        this.$parent.storeInfoForm.updateData('blAddr', res.fileData.blAddr);
                    }
                    //法定代表人
                    if(res.fileData.blPerson){
                        this.$parent.storeInfoForm.updateData('blPerson', res.fileData.blPerson);
                    }
                    //注册资本
                    if(res.fileData.blRegistCapital){
                        this.$parent.storeInfoForm.updateData('blRegistCapital', res.fileData.blRegistCapital);
                    }
                    //成立日期
                    if(res.fileData.blFundingTime){
                        this.$parent.storeInfoForm.updateData('blFundingTime', res.fileData.blFundingTime);
                    }
                    //营业期限
                    if(res.fileData.blBizLimit){
                        this.$parent.storeInfoForm.updateData('blBizLimit', res.fileData.blBizLimit);
                    }
                    //经营范围
                    if(res.fileData.blBizScope){
                        this.$parent.storeInfoForm.updateData('blBizScope', res.fileData.blBizScope);
                    }
                    //登记机关
                    if(res.fileData.blLicenseOrg){
                        this.$parent.storeInfoForm.updateData('blLicenseOrg', res.fileData.blLicenseOrg);
                    }
                    //登记日期
                    if(res.fileData.blLicenseTime){
                        this.$parent.storeInfoForm.updateData('blLicenseTime', res.fileData.blLicenseTime);
                    }
                    ...
                    this.$parent.storeInfoFormData.blImg = this.imageFile;
                } else {
                    this.warn(res.msg);
                }
                this.limitImage = false;
