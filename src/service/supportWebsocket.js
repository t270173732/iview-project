...
function connectWebsoket(id){
  ...
  store.commit('supportChat/setWebsocket', websocket)
    ...
    store.commit('supportChat/setWebsocket', '')
    console.log("退出成功")
	};
	websocket.onmessage = function(evt){
    // console.log(store.state.supportChat.historyNews.length+"条消息!!!!!!")
    console.log("连接信息："+evt.data)
    let friend = JSON.parse(evt.data)
    console.log(friend)
    if (friend.command == 11) { // 获取好友传过来的消息
      store.commit('supportChat/updateChatQueue', friend.data)
      // console.log("friend"+friend.data)
      // console.log(friend.data.from, store.state.chat.friendId, "我是两个id")
      if (friend.data.from == store.state.supportChat.friendId) {
        // 把当前消息放在当前好友的历史消息（现在的消息）
        if (friend.data.msgType == 7)
          store.commit('supportChat/setPatientInfo', friend.data);
        let newsArr = JSON.parse(JSON.stringify(store.state.supportChat.historyNews));
        // console.log(friend.data, "我是过滤出来的")
        newsArr.push(friend.data);
        store.commit('supportChat/setHistoryNews', newsArr)
      }
    } else if (friend.command == 20 && friend.code == 10018) {
      // console.log(friend, "这里是历史小i下")
      // 处理历史记录
      if(("data" in friend)) {
        store.commit('supportChat/setHistoryNews', friend.data.friends[store.state.supportChat.friendId])
      } else {
        store.commit('supportChat/setHistoryNews', [])
      }
      // router.push({
      //   path: `/inquiryOnline/${store.state.supportChat.friendId}`
      // });
      ...
      for (let i in offNews) {
        ...
        obj.newNews = number
        obj.isRead = false
        store.commit('supportChat/updateChatQueue', obj)
      }
      // PC端页面加载调用
      // console.log(store.state.supportChat.friendId,"55555555555")
      // let friendId = store.state.supportChat.friendId || store.state.supportChat.chatQueue[0].from
      // let msg = {
      //   cmd: 19,
      //   type: 1,
      //   fromUserId: friendId,
      //   userId: username
      // };
      // store.commit('supportChat/setFriendId', friendId);
      // store.state.supportChat.websocket.send(JSON.stringify(msg));
      // PC端页面加载调用
